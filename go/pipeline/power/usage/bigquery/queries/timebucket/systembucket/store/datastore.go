...
func (d *DatastoreBuckets) AddBucket(m *messages.UsageBucket, i *gbigquery.BucketType) error {
...
func (d *DatastoreBuckets) AddBuckets(m []messages.UsageBucket, ty *gbigquery.BucketType) error {
	if m == nil || len(m) == 0 {
...
func (d *DatastoreBuckets) GetBucketKey(u *messages.UsageBucket, i *gbigquery.BucketType) (*datastore.Key, error) {
	...
	name := fmt.Sprintf("%s_%s", u.SystemUID, intervalKey)

	return datastore.NameKey("SystemBucketReading", name, nil), nil
}
