...
import (
	...
	"github.com/safecility/microservices/go/device/eastronsdm/pipeline/bigquery/helpers"
	"os"
)
...
func main() {
	...
	log.Info().Msg("created bigquery topic")

	bigquerySubscription := gpsClient.Subscription(config.Topics.Bigquery)
	exists, err = bigquerySubscription.Exists(ctx)
	if !exists {
		err = createBigQuerySubscription(gpsClient, config.Subscriptions.Eastron, config.Table, bigqueryTopic)
		if err != nil {
